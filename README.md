...
| 구성 요소  | 기술 스택                                    |
|-----------|------------------------------------------|
| **Frontend** | JavaScript, CSS, Thymeleaf                   |
| **UI/UX 디자인** | 사용자 친화적인 인터페이스 설계               |
| **Backend**   | Java (version 21), Spring Boot, Spring Data JPA |
...
| 구분            | 기존 구조                                     | 개선된 구조                                  |
|-----------------|--------------------------------------------|--------------------------------------------|
| **처리 방식**   | 모든 채팅방을 하나의 토픽, 하나의 파티션에서 처리 | 채팅방 10개당 1개의 토픽을 사용, 1개의 토픽은 10개의 파티션으로 나누어 각 채팅방이 하나의 파티션을 사용 |
| **토픽 번호**   | N/A                                        | `roomId / 10 + 1` (예: `chatTopic1`, `chatTopic2`, ...) |
| **파티션 번호** | N/A                                        | `roomId % 10`                               |
| **예시**        | N/A                                        | `roomId`가 13인 경우 - **토픽**: `chatTopic2`, **파티션**: 3 |
| **미리 생성**   | N/A                                        | 기존 토픽의 5번째 파티션 사용 시 다음 토픽 미리 생성 (예: `roomId` 15 -> **토픽**: `chatTopic2`, **파티션**: 5, **chatTopic3** 미리 생성) |
